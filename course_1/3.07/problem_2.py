...
# строки одинаковой длины, на первой строке записаны символы исходного алфавита, на второй строке —
# символы конечного алфавита, после чего идёт строка, которую нужно зашифровать переданным ключом,
...
plaintext, ciphertext, encode, decode = input(), input(), list(input()), list(input())

cipher = {}

for i in range(len(plaintext)):
    cipher[plaintext[i]] = ciphertext[i]

for i in range(len(encode)):
    encode[i] = cipher[encode[i]]

for i in range(len(decode)):
    decode[i] = list(cipher.keys())[list(cipher.values()).index(decode[i])]

[print(symbol, end='') for symbol in encode]
print()
[print(symbol, end='') for symbol in decode]